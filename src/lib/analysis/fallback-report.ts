...
function extractFeeValue(text: string): string | null {
  ...
  const currencyRate = text.match(
    /\$[\d,]+(?:\.\d{2})?\s*(?:per\s+(?:day|week|month|year|diem)|\/\s*(?:day|week|month|year))\b/i,
  );
...
export function buildRuleOnlyFallbackReport(input: {
  ...
  const moneyAndFees: { label: string; value: string; evidence?: GroundedEvidenceRef[] }[] = [];
  ...
  const deadlinesAndNotice: { label: string; value: string; evidence?: GroundedEvidenceRef[] }[] = [];
